...
1. 不要把 S3 能力放回 MVP，也不要扩展出 V1.1 文档之外的协议半成品。
2. `s3_enabled=false` 时不要启动 S3 端口；V1.1 只允许显式启用独立监听。
3. 不要使用 GORM。
4. 不要引入 Redis。
5. 不要引入消息队列。
6. 不要实现复杂 RBAC。
...
S3 基础子集与缩略图已按 V1.1 独立实现；Multipart、配额、策略、回收站、搜索、分享等能力继续保留在后续路线图里，不能倒灌进 MVP 或伪装成已支持。
