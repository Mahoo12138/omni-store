...
6. `operations/image-uploads/`：图床上传的短期 `0600` 意图；`images` 提交前用于删除临时或随机最终文件，提交后只清理日志。
7. `operations/transfers/`：跨来源移动的短期 `0600` 意图与阶段标记；数据库提交前用于回滚目标，提交后用于继续删除源路径，完成后清理。
8. `logs/`：可选。1.0.0 可以只输出 stdout。
