...
当前只做存储源级访问策略。策略绑定多个普通用户，并包含多条“存储源 + 权限级别”规则。
...
普通用户的有效权限是其全部策略规则的合并结果；同一存储源命中多条规则时，
`read_write` 高于 `read_only`。超级管理员隐式拥有全部存储源的读写权限。
...
后续子路径权限继续扩展现有轻量 Policy：

1. 一个存储源可以出现在多个策略中。
2. 当前策略描述用户主体和存储源读写权限；后续增加子路径范围。
3. 不能通过嵌套存储源实现子路径权限。
