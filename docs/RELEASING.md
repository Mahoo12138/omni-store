...
7. 在隔离数据目录执行 `./scripts/verify-crash-recovery.sh` 的真实 `SIGKILL` 验收，覆盖 overwrite upload、目录复制、跨来源移动、同来源移动、移入回收站、恢复、永久删除、Multipart Complete 和图床上传。默认每类操作使用 3 个随机 kill 延迟，共 27 轮；可通过 `OMNISTORE_CRASH_ROUNDS` 增加轮数。每次重启后脚本同时核对 SQLite、来源目录、trash payload 和 operation journal，不能只检查 HTTP 恢复可用。
