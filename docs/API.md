...
跨来源移动会在系统数据目录写入短期恢复日志：复制前记录意图，目标文件和目录项完整同步后写 `target-ready`，台账、图床与分享定位在同一 SQLite 事务提交后写 `database-ready`。进程在数据库提交前退出时，下一次启动会删除目标副本并把可能已提交但尚未来得及标记的定位反向迁回来源；提交后退出则保留目标并继续删除残余源路径。恢复在 HTTP/S3 监听前完成，不能证明源或目标存在唯一完整副本时会阻止启动。存在未恢复移动时，相关存储源配置不能删除。

## 普通上传原子性与恢复

REST 上传、WebDAV `PUT`、S3 `PUT Object` 与 Multipart 最终合并共用核心文件上传路径。内容写入目标同目录的严格内部临时文件并完成 `fsync` 后，服务在 `$OMNISTORE_DATA_DIR/operations/file-uploads/` 持久化意图。覆盖已有普通文件时，旧内容先改名为同目录 `.omnistore-upload-{24位十六进制}.backup`，新内容安装并同步目录后才更新 `file_records`；数据库提交后写 `database-ready`，再删除旧备份和日志。

服务在 HTTP/S3 监听前恢复这些意图：新建上传可删除未安装的临时内容或为已安装内容补齐台账；覆盖上传可在提交前恢复旧备份，或在新目标完整时完成台账并清理旧版本。阶段标记已经存在时只清理操作独占的临时文件和备份，不按历史路径复活后来已移动或删除的文件。文件类型、大小、`mtime`、流式计算的 SHA-256 摘要、日志或阶段组合无法证明安全动作时会阻止启动并保留现场。内部临时文件与备份不会进入列表、配额或扫描台账；存在未恢复普通上传时不能删除关联存储源配置。
...
移入、恢复和永久清理均先在系统数据目录写入权限为 `0600` 的操作日志。进程在文件系统变更与 SQLite 提交之间异常退出时，服务会在下一次启动、HTTP/S3 开始监听之前，以 `trash_entries` 是否已提交为边界完成或回滚中断操作。跨文件系统复制另外记录“目标副本已完整落盘”阶段，避免把部分副本误判为可恢复数据。
