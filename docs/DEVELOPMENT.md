...
测试环境会为 `demo` 用户轮换 WebDAV Token 和一组 S3 凭据；AWS CLI 格式写入 `.testdata/s3-credentials.txt`，Playwright 共用的协议凭据写入 `.testdata/protocol-credentials.json`，两者都会在重复 seed 时重新收紧为 `0600` 并拒绝 symlink。可以用 AWS CLI 验证基础对象操作：
...
1.0 的操作日志只解决一种问题：真实文件系统状态与 SQLite 状态必须跨事务协同，而且进程中断可能造成唯一数据丢失或持久元数据分叉。新增 journal 前必须同时证明这三个条件成立，并记录文件系统提交点、SQLite 提交边界、幂等恢复方向和无法安全判断时的停机策略。

普通配置、用户、策略、凭据等只修改 SQLite 的 CRUD 继续使用数据库事务，不得仅因为操作重要就增加文件 journal。可从真实文件或可重建缓存重新扫描的数据，也不应默认引入 journal；优先使用幂等重建。当前 1.0 冻结范围只包括路径移动/永久删除、复制、跨来源移动、回收站、普通上传、S3 Multipart Part、S3 Multipart Complete 与图床上传。扩大范围必须作为独立架构决策进入后续版本，不能混入发布前缺陷修复。

恢复器必须在 HTTP/S3 监听前完成。单元测试负责穷举确定性的阶段状态；RC 还必须按[发布流程](RELEASING.md)执行真实进程 `SIGKILL` 验收，不能用继续增加 journal 替代进程级验证。
...
Playwright 默认调用 `scripts/test-env.sh run` 启动并复用 `http://127.0.0.1:18080`，覆盖未登录保护与登录退出、改密/多会话失效/管理员凭据撤销及用户删除、公开目录浏览与筛选、目录级权限、上传与搜索、新建/重命名/复制/移动、回收恢复与永久清理、图床上传/公开访问/删除、密码分享、管理员配置包下载，以及 WebDAV/S3/网页之间的文件互操作、支持方法、持久锁、S3 Multipart 和共同来源配额。协议用例必须交叉读取两个入口写入的内容，覆盖 WebDAV 基础方法及未实现方法 501、S3 Bucket/对象/List/Range/批量删除/预签名及不支持子资源 XML 501，验证 WebDAV 锁同时阻止普通 S3 PUT 与 Multipart Complete、被锁 Multipart 可在解锁后重试，验证 `507` 后无物理文件，并在 `finally` 解锁、Abort 未完成上传、恢复来源配额及清理夹具。新增会修改数据的用例必须使用唯一名称，并在成功路径末尾清理夹具。若测试服务已由外部环境管理，可设置 `OMNISTORE_E2E_BASE_URL` 跳过内置启动流程，并通过 `OMNISTORE_E2E_PROTOCOL_CREDENTIALS` 指定匹配该实例的私有协议凭据 JSON。
