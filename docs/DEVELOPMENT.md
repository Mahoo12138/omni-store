...
访问授权在未发布基线中直接使用 Policy；旧的逐用户 `user_source_permissions` 不做数据转换。
开发数据库升级后需要在“系统设置 → 访问策略”中重新授权，或重新执行测试环境 seed。
...
当前参考基线（2026-08-05）为最大 JavaScript chunk `317.57 kB`、gzip 后 `100.26 kB`，由原先的 `609.87 kB`、gzip 后 `189.88 kB` 降低而来。该数值用于识别回退，不代替按功能评估加载性能。
