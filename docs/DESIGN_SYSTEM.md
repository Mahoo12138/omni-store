...
- WebDAV、图床 API、S3 等不同协议使用连接类型切换器进入单一管理工作区；默认只挂载当前协议的查询和操作，不把所有凭据列表同时展开。切换器必须支持点击、方向键、Home / End 和清晰的选中状态。
...
- 窄屏下连接类型保持三个等宽可触达选项，隐藏辅助说明但保留协议名称和图标；工作区标题、状态与动作按阅读顺序纵向重排，凭据行转为卡片式信息流，不通过横向滚动隐藏生命周期操作，主操作在 480px 以下占满可用宽度。
