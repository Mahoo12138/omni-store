...
- 可见字段标签必须通过 `for` / `id` 指向真实表单控件；输入框与复制按钮等复合字段显式传入控件 ID，不能把标签关联到布局容器。弹窗右上角图标关闭与底部文字动作必须使用可区分的可访问名称。
...
- WebDAV、图床 API、S3 等不同协议先进入连接概览；概览只呈现用途与实时状态，不暴露重置、撤销、Bucket 等维护动作。选择协议后使用单一宽弹窗承载对应凭据管理，关闭后回到原概览入口并恢复焦点，避免在设置页内展开多个维护工作区。
- 管理弹窗依次呈现“协议与用途、状态、组级主操作、凭据或连接详情”。生成和新建是当前协议唯一的 Primary；已有 WebDAV Token 的重置降为危险弱操作。重置、撤销等立即使客户端失效的操作使用二级确认弹窗，完成后仍回到当前管理弹窗；撤销使用 `dangerGhost`，禁用/启用保持可逆的 ghost 操作。
...
- 窄屏下连接概览改为纵向入口列表，协议名称、用途与状态保持完整；管理弹窗限制在视口内并纵向滚动，标题、状态与动作按阅读顺序重排，凭据行转为卡片式信息流，不通过横向滚动隐藏生命周期操作，主操作在 480px 以下占满可用宽度。
