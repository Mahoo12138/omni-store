...
V2 阶段在上述能力上补齐复制、跨来源移动、回收站和搜索。1.0.0 仍不支持：
...
V1 阶段直接永久删除；V2 阶段完成回收站后，网页端普通删除改为进入回收站，永久清理必须使用明确的二次确认入口。
...
7. 复制和跨来源移动使用同目录临时文件写入目标，并统一经过路径锁与 WebDAV 持久锁检查。
...
V1 阶段目录列表不依赖数据库索引；V2 的全局搜索使用文件台账索引，但实时目录列表仍以真实文件系统为准。
