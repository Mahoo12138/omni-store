...
超级管理员可以修改 `public_mount_path`。MVP 中旧路径立即失效；V1.1 增加兼容重定向。
...
V1.1 起，管理员修改 `public_mount_path` 后自动保留旧路径。访问旧的 `/p/*`、`/raw/*` 或公开浏览 API 时返回 308 并跳转到当前挂载路径，同时保留源内子路径和查询参数。旧路径继续占用命名空间，避免被其他存储源复用；存储源禁用、关闭公开访问、清空挂载路径或删除后不再重定向。
