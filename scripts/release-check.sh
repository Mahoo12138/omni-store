...
echo "[1/9] Release metadata and migrations"
test -f LICENSE
...
echo "[4/9] Frontend tests and production build"
cd "$PROJECT_DIR/web"
pnpm install --frozen-lockfile
pnpm test
pnpm run build

echo "[5/9] Browser E2E"
pnpm exec playwright test

echo "[6/9] Release binary and embedded frontend"
cd "$PROJECT_DIR"
...
echo "[8/9] Compose configuration"
...
echo "[9/9] Repository diff check"
git diff --check
