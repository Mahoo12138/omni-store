# Changelog

本项目遵循 [Semantic Versioning](https://semver.org/lang/zh-CN/)。发布标签和 GitHub Release 是版本事实来源；本文件记录面向用户的重要变化，不替代完整 Git 历史。

## [Unreleased]

## [1.1.0] - 2026-09-25

### Added

- 多文件和目录上传可保留非空目录层级，显示逐文件进度，支持取消、失败重试及冲突处理。
- 文件管理新增批量移动、复制、删除、下载和分享，以及拖拽上传、目录内移动、右键菜单和排序偏好记忆。
- 最近文件和收藏入口，方便返回常用文件与文件夹。

### Changed

- 公开网盘以存储源卡片展示目录，并在同一页面布局中浏览文件。
- 移动端文件列表与网格直接显示主要操作，优化触控区域、上传提示、收藏和图床窄屏布局。

### Fixed

- 存储源读写预检失败时不再向 API/UI 暴露隐藏探测文件的绝对路径，并补充 Docker 挂载权限 FAQ。

## [1.0.0] - 2026-08-31

### Added

- 本地目录存储源、访问策略和子路径只读/读写权限。
- 私有文件管理、跨来源复制/移动、回收站、文件台账、配额和全局搜索。
- 随机文件/目录分享，可选密码、有效期、下载次数限制和撤销。
- 公开网盘及挂载路径重定向。
- WebDAV 基础读写与持久独占写锁。
- 登录/匿名图床、PicGo 接口和缩略图缓存。
- 可选 S3 Path-style API、Signature V4、多个凭据和 Multipart Upload。
- 审计日志、系统配置包导出、隔离测试环境和自动化 E2E。
- 使用 MIT License 公开发布。

### Security

- 统一路径规范化、路径穿越与 symlink 拒绝、排除规则、CSRF、可信代理和写入锁检查。
- 密码与 Token 只保存哈希，S3 Secret 使用 AES-256-GCM 加密。

### Fixed

- 表单字段的可见标签现在通过 `for/id` 与输入控件关联，改善键盘操作和辅助技术识别。
- 私有文件管理器下载不再打开多余的浏览器标签页；文件预览与外部链接仍按语义新开页面。
- 管理端审计日志统一按每页 10 条分页，接口返回明确的分页元数据，并将动作、主体和入口类型显示为中文。
- 管理端存储源列表收紧首列和行间距，减少宽屏中间留白，同时保留窄屏横向查看长路径的能力。
- 分享与图床页面统一使用标准内容边距；图床信息改为从图床目标面板打开弹窗查看，并移除重复的 PicGo 使用说明区。

`1.0.0` 由 `v1.0.0` 稳定标签发布。后续用户可见变化请记录在新的 Unreleased 区段，并按下一目标 SemVer 组织迁移与发布。
